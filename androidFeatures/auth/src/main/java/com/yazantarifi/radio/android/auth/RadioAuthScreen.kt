    ...
    private fun onRequestApiAccessToken(accessToken: String, loadingView: View) {
        authUseCase.execute(accessToken, object: SopifyUseCaseListener {
            override fun onStateUpdated(newState: SopifyState) {
                when (newState) {
                    is SopifyEmptyState -> {}
                    is SopifyErrorState -> onShowErrorMessage(newState.exception.message ?: "Unexpected Error")
                    is SopifySuccessState -> onAccountLoggedInMessage(newState.payload as SpotifyAuthResponse)
                    is SopifyLoadingState -> lifecycleScope.launch(Dispatchers.Main) {
