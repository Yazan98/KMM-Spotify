...
import com.yazantarifi.kmm.sopy.SopifyConstraintsException
...
import com.yazantarifi.kmm.sopy.base.useCases.constraints.SopifyType
...
abstract class SopifyUseCase<RequestValue, ResponseValue : Any>: CoroutineScope, SopifyUseCaseType {
    ...
    protected fun onSubmitLoadingState(isLoading: Boolean) {
        launch(coroutineScope) {
            listener?.onStateUpdated(SopifyState.SopifyLoadingState(isLoading))
        }
    }

    protected fun onSubmitExceptionState(throwable: Throwable) {
        launch(coroutineScope) {
            listener?.onStateUpdated(SopifyState.SopifyErrorState(throwable))
        }
    }

    protected fun onSubmitSuccessState(response: ResponseValue) {
        launch(coroutineScope) {
            listener?.onStateUpdated(SopifyState.SopifySuccessState(response))
        }
    }
